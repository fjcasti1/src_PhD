...
c     nmax is needed in work arrays in spectral_tools.
      ...
      real*8 dx1(0:nx,0:nx),dz1(0:nz,0:nz),dx2(0:nx,0:nx),dz2(0:nz,0:nz)
     &       ,dr1(0:nr,0:nr,2),dr2(0:nr,0:nr,2),lap(0:nr,0:nr,2)
...
C <INITIALIZATION> ----------------------------------------------------------
      ...
      call deriv(dx1,dz1,dx2,dz2,dr1,dr2,lap,x,z,r,ri,riq,nr,nz)
...
c     stage(1) is the solution at N, stage(2) at N-1
         call time_stepper(Bo,Re,Ro,wf,dt,pnu,rad,alt,gama,eta,
     &        stage,tps,tdr,tir,tdi,tii,wk,dz1,dz2,nr,nz,nn,
     &        dr1,lap,z,r,ri,axd,vxd,bxd,azd,vzd,bzd,axn,vxn,bxn,azn,
     &        vzn,bzn,cx01,cx03,cz01,cz02,cz03,cz11,cz12,cz13,vr,
     &        vt,vz,nlvr,nlvt,nlvz,rotvorz,rotvorr)
c     stage(1) is the solution at N, stage(2) at N+1
...
C </MAIN LOOP> ---------------------------------------------------------
      close(OUT_TS_UNIT)
      stop
      end program main_unifTop

c=======================================================================

      subroutine bcvel(tps,r,z,vr,vt,vz,rad,nr,nz,nn,
     &                 Bo,Re,Ro,wf,pnu,gama,eta)   ! diffrot
      implicit none
      integer nr,nz,nn,n,i,j
      real*8 vr(0:nr,0:nz,0:nn-1),vt(0:nr,0:nz,0:nn-1)
      real*8 vz(0:nr,0:nz,0:nn-1),r(0:nr),z(0:nz),tps
      real*8 rad,Bo,Re,Ro,wf,pnu,gama,eta, mu

c     r(i)=cos(pi*i/nx) in (0,1], radial collocation points

c---- First, no-splip conditions in all boundaries, in all Fourier modes
      do n=0,nn-1
         do i=0,nr
            vr(i,0,n)=0.d0; vt(i,0,n)=0.d0; vz(i,0,n)=0.d0 ! Top
            vr(i,nz,n)=0.d0; vt(i,nz,n)=0.d0; vz(i,nz,n)=0.d0 !Bottom
         enddo
         do j=1,nz-1
            vr(0,j,n)=0.d0; vt(0,j,n)=0.d0; vz(0,j,n)=0.d0 ! Sidewall
         enddo
      enddo

c---- Now, the non-zero boundary conditions

c     Top lid: Uniform rotation with rate Omega rad/s
      do i=0,nr
         vt(i,0,0)=-r(i)*(pnu*Re/rad)*(1d0+Ro*dsin(wf*tps))
      enddo

c     Side wall: no-slip BC with a gaussian regularization to mantain
c     contuniuity
      mu = gama ! Center of the Gaussian
      do j=1,nz-1
         vt(0,j,0)=-(pnu*Re/rad)*(1d0+Ro*dsin(wf*tps))*
     &               dexp(-(z(j)-mu)**2d0/(2d0*eta**2d0))
      enddo
      return
...
C     Start from rest with random perturbation in vr
         ...
         if (imode.eq.0) then
           do j=0,nz
             do i=0,nr
c               vr(i,j,0,1)=gasdev(m)*pert
c               vr(i,j,0,2)=gasdev(m)*pert
               call random_number(unifRand)
               vr(i,j,0,1)=(2*unifRand-1)*pert
               call random_number(unifRand)
               vr(i,j,0,2)=(2*unifRand-1)*pert
             enddo
           enddo
         else
           do j=0,nz
             do i=0,nr
c               vr(i,j,2*imode-1,1)=gasdev(m)*pert
c               vr(i,j,2*imode,  1)=gasdev(m)*pert
c               vr(i,j,2*imode-1,2)=gasdev(m)*pert
c               vr(i,j,2*imode,  2)=gasdev(m)*pert
               call random_number(unifRand)
               vr(i,j,2*imode-1,1)=(2*unifRand-1)*pert
               call random_number(unifRand)
               vr(i,j,2*imode,  1)=(2*unifRand-1)*pert
               call random_number(unifRand)
               vr(i,j,2*imode-1,2)=(2*unifRand-1)*pert
               call random_number(unifRand)
               vr(i,j,2*imode,  2)=(2*unifRand-1)*pert
             enddo
           enddo
         endif
      elseif (ibegin.eq.1) then
...
C     Continue restart solution with random perturbation, set t=0.
         ...
         if (imode.eq.0) then
            do j=0,nz
               do i=0,nr
c                  vr(i,j,0,1)=vr(i,j,0,1)+gasdev(m)*pert
c                  vr(i,j,0,2)=vr(i,j,0,2)+gasdev(m)*pert
                  call random_number(unifRand)
                  vr(i,j,0,1) = vr(i,j,0,1) + (2*unifRand-1)*pert
                  call random_number(unifRand)
                  vr(i,j,0,2) = vr(i,j,0,2) + (2*unifRand-1)*pert
               enddo
            enddo
         else
            do j=0,nz
              do i=0,nr
c                vr(i,j,2*imode-1,1)=vr(i,j,2*imode-1,1)+gasdev(m)*pert
c                vr(i,j,2*imode,  1)=vr(i,j,2*imode,  1)+gasdev(m)*pert
c                vr(i,j,2*imode-1,2)=vr(i,j,2*imode-1,2)+gasdev(m)*pert
c                vr(i,j,2*imode,  2)=vr(i,j,2*imode,  2)+gasdev(m)*pert
                call random_number(unifRand)
...
c=======================================================================
      ...
      subroutine time_stepper(Bo,Re,Ro,wf,dt,pnu,rad,alt,gama,eta,
     &     stage,tps,tdr,tir,tdi,tii,wk,dz1,dz2,nr,nz,nn,
     &     dr1,lap,z,r,ri,axd,vxd,bxd,azd,vzd,bzd,axn,vxn,bxn,azn,
     &     vzn,bzn,cx01,cx03,cz01,cz02,cz03,cz11,cz12,cz13,vr,
     &     vt,vz,nlvr,nlvt,nlvz,rotvorz,rotvorr)
c     stage(1) points to N, stage(2) points to N-1 on input, the opposite on output
      ...
      real*8  dz1(0:nz,0:nz),dz2(0:nz,0:nz),dr1(0:nr,0:nr,2)
      real*8  lap(0:nr,0:nr,2),z(0:nz),r(0:nr),ri(0:nr)
     ...
     &        cz11,cz12,cz13(1:nz-1)
      ...
      real*8  dt,pnu,rad,alt,tps,Bo,Re,Ro,wf,gama,eta
      real*8  alpha,b,a
...
c     boundary conditions for the velocity field
      call bcvel(tps,r,z,wk(0,0,0,4),wk(0,0,0,6),wk(0,0,0,3),rad,
     &          nr,nz,nn,Bo,Re,Ro,wf,pnu,gama,eta)
!$OMP PARALLEL
!$OMP+SHARED(alpha,wk)
!$OMP+PRIVATE(n,i)
!$OMP DO
...
!$OMP END DO
!$OMP END PARALLEL
c     hacemos la derivada respecto de z de v_theta; PARCIAL z DE vr(N+1)
      alpha=2d0/alt
      do k=0,nn1
